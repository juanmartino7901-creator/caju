    ...
    <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 14 }}>
      <Card>
        ...
        {editing ? <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <Input label="Subtotal" type="number" value={form.subtotal} onChange={e => setForm(f => { const subtotal = e.target.value; return { ...f, subtotal, total: (Number(subtotal) || 0) + (Number(f.tax_amount) || 0) }; })} />
          <Input label="IVA" type="number" value={form.tax_amount} onChange={e => setForm(f => { const tax_amount = e.target.value; return { ...f, tax_amount, total: (Number(f.subtotal) || 0) + (Number(tax_amount) || 0) }; })} style={inv.confidence?.tax_amount != null && inv.confidence.tax_amount < 0.8 ? { borderColor: "#dc2626", background: "#fff5f5" } : {}} />
          <Input label="Total (subtotal + IVA)" type="number" value={form.total} onChange={e => setForm(f => ({ ...f, total: e.target.value }))} style={inv.confidence?.total != null && inv.confidence.total < 0.8 ? { borderColor: "#dc2626", background: "#fff5f5" } : {}} />

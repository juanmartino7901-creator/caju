import type {
  ProjectData, MonthlyProjection, RevenueLineItem, CostLineItem, LoanPayment, PaymentTerms,
} from './types';
...
function getYearlyCompoundFactor(rates: number[], upToYear: number): number {
  let factor = 1;
...
function getInflationFactor(inflationRates: number[], upToYear: number): number {
...
): { month: number; principalPayment: number; interestPayment: number }[] {
  const schedule: { month: number; principalPayment: number; interestPayment: number }[] = [];
...
export function getMonthlyExchangeRate(setup: { exchangeRates?: number[]; exchangeRate?: number; horizonMonths: number }, month: number): number {
  const rates = setup.exchangeRates;
  if (!rates || rates.length === 0) return setup.exchangeRate ?? 1;
  // Year-end values: rates[0] is end of year 1 (month 11), rates[1] is end of year 2 (month 23), etc.
  // Month 0 starts at rates[0] (or exchangeRate fallback as "year 0 end")
  // Linear interpolation between year-end points
  const startRate = setup.exchangeRate ?? rates[0] ?? 1; // rate at month 0
  // Build anchor points: month 0 = startRate, month 11 = rates[0], month 23 = rates[1], ...
  const anchors: { month: number; rate: number }[] = [{ month: 0, rate: startRate }];
...
export function projectCashFlows(data: ProjectData): MonthlyProjection[] {
  ...
  // Revenue accrued per stream per month (converted to local currency)
  const accruedRevenue: number[][] = revenueStreams.map(stream => {
    ...
    for (let m = 0; m < H; m++) {
      ...
      if (stream.useProducts && stream.products.length > 0) {
        ...
        for (const product of stream.products) {
          ...
          // Convert USD-denominated revenue to local currency
  ...
  // For product-mode streams, blend product payment terms weighted by revenue
  const cashRevenue: number[][] = revenueStreams.map((stream, si) => {
    if (stream.useProducts && stream.products.length > 0) {
      // Per-product accrued and cash, then sum
      const productCash: number[][] = stream.products.map(product => {
  ...
  // Variable costs accrued per VC per month (based on accrued revenue)
  const accruedVC: number[][] = variableCosts.map(vc => {
    const arr = new Array(H).fill(0);
    for (let m = 0; m < H; m++) {
  ...
  const cashVC: number[][] = variableCosts.map((vc, vi) =>
    applyPaymentTerms(accruedVC[vi], vc.paymentTerms ?? DEFAULT_PAYMENT_TERMS, H).map(v => Math.round(v))
  );

  // --- Compute AR and AP balances ---
  // AR at end of month M = accrued revenue not yet collected
  // AP at end of month M = accrued costs not yet paid
  function computeOutstandingBalance(
    accrued: number[][],
    items: { paymentTerms?: PaymentTerms }[],
    horizonMonths: number
  ): number[] {
    ...
    for (let si = 0; si < accrued.length; si++) {
      ...
      const pct60 = (terms.pct60 ?? 0) / 100;
      for (let m = 0; m < horizonMonths; m++) {
        // Outstanding from month M: portions to be collected in M+1 and M+2
        balance[m] += accrued[si][m] * (pct30 + pct60);
        // Outstanding from month M-1: 60-day portion still not collected
  ...
  // For product-based streams, we need to aggregate product-level accruals
  const productAccruedByStream: number[][] = revenueStreams.map((stream, si) => {
    if (stream.useProducts && stream.products.length > 0) {
      // Return stream-level accrued (already computed)
      return accruedRevenue[si];
    }
    return accruedRevenue[si];
  });

  // For AR, we need per-product detail for product-mode streams
  ...
  for (let m = 0; m < H; m++) {
    ...
    const revenueLines: RevenueLineItem[] = revenueStreams.map((stream, si) => ({
    ...
    const variableCostLines: CostLineItem[] = variableCosts.map((vc, vi) => ({
    ...
    // Fixed Costs (no payment terms)
    const fixedCostLines: CostLineItem[] = fixedCosts.map(fc => {
      ...
      // Convert USD-denominated costs to local currency
    ...
    // Employees
    const employeeCostLines: CostLineItem[] = employees.map(emp => {
      ...
      // Convert USD-denominated salaries to local currency
    ...
    // CapEx
    const capexLines: CostLineItem[] = capexItems
    ...
    // Loan Payments
    const loanPayments: LoanPayment[] = loans.map((loan, idx) => {
...
export function computeNPV(projections: MonthlyProjection[], annualDiscountRate: number): number {
...
export function findPeakDeficit(projections: MonthlyProjection[]): number {
  return Math.min(...projections.map(p => p.closingCash));
}

export function computePaybackPeriod(projections: MonthlyProjection[], initialInvestment: number): number | null {
...
export function getAnnualSummary(projections: MonthlyProjection[]): MonthlyProjection[] {
  const years = new Map<number, MonthlyProjection>();
  for (const p of projections) {
    if (!years.has(p.year)) {
